...
var parseCoupons = function(coupons, domain) {
  ...
  try{
    ...
    +'<h1>Coupon Saver</h1><p>Browse coupons below that have been used for <strong>'+domain+'</strong></p>'
    +'<p style="font-style:oblique;">Click any coupon to copy &amp; use</p>'
    +'<ul>'+couponHTML+'</ul>';
...
var createEvents = function(){
  ...
  document.querySelector('._submit-overlay  .submit-coupon').addEventListener('click', function(event){
    ...
    submitCoupon(code, desc, getDomain(window.location.hostname));
  });
